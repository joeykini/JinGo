...
Rectangle {
    ...
    Connections {
        ...
        function onIsLoadingChanged() {
            // 【关键修复】当加载完成时，强制清空并重新加载数据
            // 这是用户手动刷新触发的，需要完全重建
            if (!serverListViewModel.isLoading && isInitialized) {
                Qt.callLater(function() {
                    loadServersToModel(true)  // 强制清空
                    serverCount = serversModel.count
                    groupServersByCountry()
                })
            }
    ...
    // ⭐ 已移除对 SubscriptionManager.batchUpdateCompleted 的监听
    // 服务器列表会通过 ServerListViewModel.serversChanged 自动更新，无需额外监听
